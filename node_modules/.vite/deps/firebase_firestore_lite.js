...
var __PRIVATE_ExponentialBackoff = class {
  ...
  K(t) {
    ...
    n > 0 && __PRIVATE_logDebug("ExponentialBackoff", `Backing off for ${n} ms (base delay: ${this.k} ms, delay with jitter: ${e} ms, last attempt: ${r} ms ago)`), this.U = this.B.enqueueAfterDelay(this.timerId, n, () => (this.j = Date.now(), t())), // Apply backoff factor to determine next delay and ensure it is within
    // bounds.
...
var __PRIVATE_DatastoreImpl = class extends class Datastore {
  ...
  constructor(t, e, r, n) {
    super(), this.authCredentials = t, this.appCheckCredentials = e, this.connection = r, this.serializer = n, this.J = false;
  }
  Y() {
    if (this.J)
  ...
  R(t, e, r) {
    return this.Y(), Promise.all([this.authCredentials.getToken(), this.appCheckCredentials.getToken()]).then(([n, i]) => this.connection.R(t, e, r, n, i)).catch((t2) => {
  ...
  g(t, e, r, n) {
    return this.Y(), Promise.all([this.authCredentials.getToken(), this.appCheckCredentials.getToken()]).then(([i, s]) => this.connection.g(t, e, r, i, s, n)).catch((t2) => {
      throw "FirebaseError" === t2.name ? (t2.code === p && (this.authCredentials.invalidateToken(), this.appCheckCredentials.invalidateToken()), t2) : new FirestoreError(A, t2.toString());
    });
  }
  terminate() {
...
async function __PRIVATE_invokeRunAggregationQueryRpc(t, e, r) {
  var n;
  const i = __PRIVATE_debugCast(t), { request: s, Z: o } = function __PRIVATE_toRunAggregationQueryRequest(t2, e2, r2) {
    const n2 = __PRIVATE_toQueryTarget(t2, e2), i2 = {}, s2 = [];
    let o2 = 0;
    return r2.forEach((t3) => {
      const e3 = "aggregate_" + o2++;
      i2[e3] = t3.alias, "count" === t3.aggregateType ? s2.push({
        alias: e3,
        count: {}
      }) : "avg" === t3.aggregateType ? s2.push({
        alias: e3,
        avg: {
          field: __PRIVATE_toFieldPathReference(t3.fieldPath)
        }
      }) : "sum" === t3.aggregateType && s2.push({
        alias: e3,
        sum: {
          field: __PRIVATE_toFieldPathReference(t3.fieldPath)
        }
...
var __PRIVATE_ParseContextImpl = class ___PRIVATE_ParseContextImpl {
  ...
  it(t) {
    ...
    if (__PRIVATE_isWrite(this.tt) && U.test(t))
...
function __PRIVATE_parseSetData(t, e, r, n, i, s = {}) {
  const o = t.lt(s.merge || s.mergeFields ? 2 : 0, e, r, i);
...
var __PRIVATE_ArrayUnionFieldValueImpl = class extends FieldValue {
  ...
  _toFieldTransform(t) {
    const e = __PRIVATE_createSentinelChildContext(
      ...
      /*array=*/
      true
    ), r = this.ht.map((t2) => __PRIVATE_parseData(t2, e)), n = new __PRIVATE_ArrayUnionTransformOperation(r);
...
var __PRIVATE_ArrayRemoveFieldValueImpl = class extends FieldValue {
  ...
  _toFieldTransform(t) {
    const e = __PRIVATE_createSentinelChildContext(
      ...
      /*array=*/
      true
    ), r = this.ht.map((t2) => __PRIVATE_parseData(t2, e)), n = new __PRIVATE_ArrayRemoveTransformOperation(r);
    return new FieldTransform(t.path, n);
  }
  isEqual(t) {
    return this === t;
...
function __PRIVATE_parseUpdateVarargs(t, e, r, n, i, s) {
  const o = t.lt(1, e, r), a = [__PRIVATE_fieldPathFromArgument$1(e, n, r)], u = [i];
  if (s.length % 2 != 0)
...
function __PRIVATE_parseQueryValue(t, e, r, n = false) {
  return __PRIVATE_parseData(r, t.lt(n ? 4 : 3, e));
}
function __PRIVATE_parseData(t, e) {
  ...
  return function __PRIVATE_parseScalarValue(t2, e2) {
    ...
    if (t2 instanceof DocumentReference) {
      ...
      if (!n.isEqual(r))
        throw e2.ut(`Document reference is for database ${n.projectId}/${n.database} but should be for database ${r.projectId}/${r.database}`);
      return {
        referenceValue: __PRIVATE_toResourceName(t2.firestore._databaseId || e2.databaseId, t2._key.path)
      };
    }
    throw e2.ut(`Unsupported field value: ${__PRIVATE_valueDescription(t2)}`);
  }(t, e);
}
function __PRIVATE_parseObject(t, e) {
  ...
  return !function isEmpty(t2) {
    ...
    const i = __PRIVATE_parseData(n, e.rt(t2));
    null != i && (r[t2] = i);
  }) : (
...
var __PRIVATE_TransactionRunner = class {
  constructor(t, e, r, n, i) {
    this.asyncQueue = t, this.datastore = e, this.options = r, this.updateFunction = n, this.deferred = i, this.ft = r.maxAttempts, this.Et = new __PRIVATE_ExponentialBackoff(
      this.asyncQueue,
      "transaction_retry"
...
var __PRIVATE_AsyncQueueImpl = class {
  constructor() {
    ...
    this.Vt = [], // Is this AsyncQueue being shut down? Once it is set to true, it will not
    // be changed again.
    this.yt = false, // Operations scheduled to be queued in the future. Operations are
    // automatically removed after they are run or canceled.
    this.wt = [], // visible for testing
    this.gt = null, // Flag set while there's an outstanding AsyncQueue operation, used for
    // assertion sanity-checks.
    this.Ft = false, // Enabled during shutdown on Safari to prevent future access to IndexedDB.
    this.vt = false, // List of TimerIds to fast-forward delays for.
    this.Dt = [], // Backoff timer used to schedule retries for retryable operations
    this.Et = new __PRIVATE_ExponentialBackoff(
      this,
      "async_queue_retry"
    ...
    this.bt = () => {
      ...
      t2 && __PRIVATE_logDebug("AsyncQueue", "Visibility state changed to " + t2.visibilityState), this.Et.H();
    };
    const t = getDocument();
  ...
  async Nt() {
    if (0 !== this.Vt.length) {
      ...
      this.Vt.length > 0 && // If there are additional operations, we re-schedule `retryNextOp()`.
  ...
  St(t) {
    const e = this.It.then(() => (this.Ft = true, t().catch((t2) => {
      this.gt = t2, this.Ft = false;
      const e2 = (
        /**
  ...
  Qt(t) {
    return this.qt().then(() => {
      this.wt.sort((t2, e) => t2.targetTimeMs - e.targetTimeMs);
      for (const e of this.wt)
